...
test('unpackWire round-trips an envelope and reads a bare string as text', () => {
  ...
  assert.deepEqual(round, { text: 'yo', threadId: 'T', replyTo: 'R', priority: 'low', expiresAt: null, attachments: [] });

  assert.deepEqual(unpackWire('just text'), { text: 'just text', threadId: null, replyTo: null, priority: null, expiresAt: null, attachments: [] });
});
...
test('unpackWire never mistakes ordinary JSON for an envelope (no _tgv marker)', () => {
  ...
  assert.deepEqual(unpackWire(literal), { text: literal, threadId: null, replyTo: null, priority: null, expiresAt: null, attachments: [] });
