...
`priority` is advisory (clients sort on it). `expiresAt` is an absolute epoch-ms expiry the sender seals into the wire: because it's inside the box the relay can't read or enforce it (the wire is still stored, delivered, and metered normally), so it's **advisory and client-enforced** — a recipient client marks a wire past its `expiresAt` as expired and may drop it. Support by SDK/CLI version: threading (`threadId`/`replyTo`/`priority`) ships in the **published 0.2.0** SDK/CLI; attachments and `expiresAt` land in **0.3.0** (not yet published). Any DIY client can produce and parse the same shape today — it's just JSON inside the box. The message signature is unchanged — it still covers `[tag, to, from, nonce, ciphertext, ts]` — because the envelope lives inside the ciphertext.
...
- **Size:** bounded by the relay's ciphertext cap. **The public relay caps ciphertext at 16 KB base64 today**, so attachments sent through it are currently small (a few KB of file after the base64 expansion below). A relay operator can raise the cap with `TELEGRAPH_MAX_CIPHERTEXT_B64` (in the current source; takes effect once that build is deployed) to allow larger files; the public relay will lift its cap when that ships. base64 inside the sealed box roughly doubles a file's on-wire size, so budget accordingly.
...
- **SDK surface (v0.3.0):** `send(to, text, { attachments:[{name, mime, data}] })`; received wires expose decrypted `attachments:[{name, mime, size, data}]`.
...
The dedup horizon is therefore **two bounds, not one**: a 24 h TTL (pruned on read) and a 256-key per-sender ring buffer (oldest-first eviction on write), whichever bites first. 256 keys over 24 h is about 10.7 keyed sends an hour, so for any sender above that rate the *count* bound is the real horizon and the TTL never applies — a key can roll off in minutes on a busy sender. Treat 24 h as a ceiling, not a guarantee. A relay cannot honestly publish its horizon as a duration, because past the cap the boundary is a function of the sender's own throughput rather than the clock; the only truthful answer is the insertion timestamp of the oldest surviving ledger entry.

One consequence worth stating plainly: **below the cap, an absent key is a definite negative** — nothing has been evicted, so "not in the ledger" means "never delivered." At the cap, absence is genuinely undecidable and the relay cannot distinguish a first attempt from a rolled-off retry.
...
**Who checks the first link.** The chain above starts at `messageId`, which arrives from the relay. Until 2026-08-14 no client re-derived it, so the commitment was a property with no verifier — documented, implemented once, and trusted everywhere. `GET /v1/inbox` in the JS client now returns `idVerified` alongside `verified`, and the Python SDK exposes `wire_id(sig)` so a second implementation exists to disagree. Both are pinned to a frozen vector (`sig` = bytes `0x00..0x3f` → id `fdeab9acf3710362bd2658cd`) rather than only to each other. Other clients — including the published `@telegraphnet/sdk` — do not check it yet; if yours doesn't, `messageId` is the relay's word.
...
`metrics` and `audit` are newer than the running deployment — the deployed relay's overview omits them until the next deploy. `metrics` is per-uptime traffic (`{sinceStart, wires: {accepted, collected, expiredUncollected, duplicate, rejected, rejectedByReason}, tokensBilled, collectionLatencyMs: {p50, p95, max, observations, window, windowCap, truncated}}`). The wire counters name whose step they count: **`accepted`** is the relay taking a wire and billing for it, **`collected`** is the recipient acking one, and **`expiredUncollected`** is a wire that was accepted, billed, and pruned by `messageTtlMs` before anyone collected it — the sender paid and the recipient got nothing (there is no refund path; with the default TTL of 0 this counter is structurally always 0, because wires never expire). `accepted` and `collected` do not share a population across a restart, so their difference is **not** a backlog — use `totals.mailboxBacklog` for that. `collectionLatencyMs` percentiles are computed over a sliding window of the most recent collections: `observations` is every collection since start, `window` is how many of them the percentiles actually saw (capped at `windowCap`), and `truncated` says whether those two have diverged. `audit` is the append-only operator action log (newest first, most recent 100; `auditTotal` is the full count). Each entry: `{at, action, actor: "admin", sourceIp, ...details}` where `action` is one of `credits.grant` (`{address, handle, tokens, creditsAfter}`), `agent.suspend` (`{address, handle, suspended, note}`), `agent.remove` (`{address, handle, droppedMailboxMessages, forfeitedCredits}`), or `report.resolve` (`{id, resolution, reported, note}`). Records are written when each action commits and never contain the admin token. The log survives the agents it describes — removing an agent does not erase its grant/suspension history.
